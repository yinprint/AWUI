...
public class TabLayout  extends MyPanel {
	...
	public TabLayout(int width,int height){
		...
		label=new TextView("����ϵͳ",JLabel.CENTER);
		...
		listview.setOnItemListener(new OnItemListener(){
			...
			public void onItem(int position, Component view) {
				...
				if(clicked!=position){
					 ...
					 if(clicked!=-1) items[clicked].getView().setVisible(false);
					clicked=position;
					 items[position].getView().setVisible(true);
			 	//Content.removeAll();
	...
	public void setItems(Fragment[] it){
		...
		if(items!=null&&items.length>0){
			 for(int m=0;m<items.length;m++){
				 Content.add(items[m].getView());
				 
			 }
			 items[0].getView().setVisible(false);
		}
